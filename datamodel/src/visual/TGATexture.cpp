...
void TextureImplementation::parseV1(rcbe::binary::BinaryBuffer &&bb) {
    ...
    BOOST_LOG_TRIVIAL(debug) << "ID length value: " << static_cast<int>(id_length);
    const auto color_map_type = bb.at(offset, sizeof(uint8_t)).get<uint8_t>();
    offset += sizeof(uint8_t);
    BOOST_LOG_TRIVIAL(debug) << "Color map type: " << static_cast<int>(color_map_type);
    const auto image_type = bb.at(offset, sizeof(uint8_t)).get<uint8_t>();
    offset += sizeof(uint8_t);
    BOOST_LOG_TRIVIAL(debug) << "Image type: " << static_cast<int>(image_type);

    color_map_spec_ = bb.at(offset, ColorMapSpecification::SIZE).get<ColorMapSpecification>();
    offset += ColorMapSpecification::SIZE;

    BOOST_LOG_TRIVIAL(debug) << "Color Map Spec " << color_map_spec_.first_entry_index << " " << color_map_spec_.color_map_length << " "
    << static_cast<int>(color_map_spec_.color_map_entry_size);
    ...
    BOOST_LOG_TRIVIAL(debug) << "W: " << image_spec_.width << " H: " << image_spec_.height << " Pixel depth " << static_cast<int>(image_spec_.pixel_depth);

    if (color_map_type == 0) {
        BOOST_LOG_TRIVIAL(warning) << "No color map data available!";

        const auto total_pixels = image_spec_.width * image_spec_.height;
        BOOST_LOG_TRIVIAL(debug) << "Total pixels in image " << total_pixels;
        const auto pixel_depth_bytes = image_spec_.pixel_depth / 8;

        BOOST_LOG_TRIVIAL(debug) << "Image body size is " << image_body_.size();
    ...
    BOOST_LOG_TRIVIAL(debug) << "Image body size " << image_body_.size();
}
